...
export class NextJSPlugin extends AnalyzerPlugin {
  ...
  async detectRouterType(projectPath) {
    const appDir = existsSync(join(projectPath, 'app')) || existsSync(join(projectPath, 'src/app'));
    const pagesDir = existsSync(join(projectPath, 'pages')) || existsSync(join(projectPath, 'src/pages'));
  ...
  async discoverAppRoutes(projectPath) {
    ...
    for (const appDir of appDirs) {
      ...
      if (!existsSync(fullAppDir)) {continue;}
      ...
      for (const layoutFile of layoutFiles) {
        ...
        const routePath = this.convertAppRouteToPath(relativePath.replace('/layout', ''));
  ...
  async discoverPageRoutes(projectPath) {
    ...
    for (const pagesDir of pagesDirs) {
      ...
      if (!existsSync(fullPagesDir)) {continue;}
    ...
    for (const pattern of componentPatterns) {
      ...
      for (const componentFile of componentFiles) {
        ...
        components.push({
          ...
          isUtility: relativePath.includes('util') || relativePath.includes('helper'),
  ...
  async hasLoading(dirPath) {
    const loadingFiles = ['loading.js', 'loading.jsx', 'loading.ts', 'loading.tsx'];
  ...
  async hasNotFound(dirPath) {
    const notFoundFiles = ['not-found.js', 'not-found.jsx', 'not-found.ts', 'not-found.tsx'];
